...
const BenchStaffNavbar = () => {
  const [dropdownOpen, setDropdownOpen] = useState(false);

  const navigate=useNavigate()
  const dispatch=useDispatch()

  // *********************Find The Logged In Bench Staff data************************************
  const storedData=JSON.parse(localStorage.getItem("benchStaff")) || []
  const username=useSelector((state)=>state.auth.user)
  const foundStaff=storedData.find((staff)=>staff.name===username)
  ...
  return (
    <nav className="bg-green-600 text-white h-14 flex items-center justify-between px-4 md:px-8 shadow-md ">
      ...
      <div className="hidden sm:flex flex-col items-center text-center">
            <span className="text-base font-medium">Name:- {username}</span>
            <span className="text-xs">Location:-{foundStaff.address}</span>
      </div>
...
export default BenchStaffNavbar;
